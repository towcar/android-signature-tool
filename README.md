...
Android Signature Tool is an Android library for drawing smooth signatures. It uses variable width Bézier curve interpolation based on [Smoother Signatures](http://corner.squareup.com/2012/07/smoother-signatures.html) post by [Square](https://squareup.com).

![Screenshot](https://github.com/somethingelselabs/android-signaturetool/raw/master/header.png)
...
```gradle
compile 'com.github.somethingelselabs:signature-tool:1.2.1'
```
...
```xml
<dependency>
  <groupId>com.github.somethingelselabs</groupId>
  <artifactId>signature-tool</artifactId>
...
*Please see the `/SignatureTool-Example` app for a more detailed code example of how to use the library.*

1. Add the `SignatureTool` view to the layout you want to show.
```xml
 <com.github.somethingelselabs.signaturetool.views.SignatureTool
     xmlns:android="http://schemas.android.com/apk/res/android"
     xmlns:app="http://schemas.android.com/apk/res-auto"
     android:id="@+id/signature_tool"
 ...
 * `clearOnDoubleClick` - Double click to clear tool (default: false)
...
3. Configure signature events listener
 ...
 mSignatureTool = (SignatureTool) findViewById(R.id.signature_tool);
 mSignatureTool.setOnSignedListener(new SignatureTool.OnSignedListener() {

      @Override
      public void onStartSigning() {
          //Event triggered when the tool is touched
      }

     @Override
     public void onSigned() {
         //Event triggered when the tool is signed
     }

     @Override
     public void onClear() {
         //Event triggered when the tool is cleared
     }
 });
 ```
...
The `SignatureTool` view has custom Data Binding attribute setters for all the listener events:
